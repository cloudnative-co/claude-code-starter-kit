...
STR_CODEX_RESTART_HINT="If Claude Code is already running, use a new terminal or next session to confirm Codex MCP is available"
STR_CODEX_SETUP_DONE="Codex MCP setup complete"
STR_CODEX_SETUP_INCOMPLETE="Codex MCP setup is not complete yet. Finish the remaining steps later"
STR_CODEX_SETUP_CONFIRM="Start Codex MCP setup? (Supports ChatGPT sign-in or OpenAI API key auth)"
...
STR_DRYRUN_CONFLICT_REMEMBERED="(remembered)"
STR_DRYRUN_CONFLICT_WILL_PROMPT="→ will prompt in real run"
...
STR_EXISTING_CLAUDE_SIDE_EFFECTS="Optional side effects like Ghostty, fonts, shell RC updates, or Codex MCP setup may also run depending on your selections."
