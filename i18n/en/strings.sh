...
# Step 4: Editor
STR_EDITOR_TITLE="Which editor do you use? (recorded in saved config)"
...
STR_HOOKS_SAFETY_NET="Safety Net - Helps prevent accidental destructive git/filesystem commands (cc-safety-net)"
...
STR_HOOKS_TMUX="Tmux Reminder - Suggest run_in_background for foreground dev servers (non-blocking)"
STR_HOOKS_DOC_BLOCK="Doc Blocker - Confirm ad-hoc SUMMARY/REPORT style docs (general docs pass)"
...
STR_WCE_NODE_HINT="Install Node.js 22.19+ then run: (cd ~/.claude/skills/web-content-extraction && npm ci --omit=dev)"
