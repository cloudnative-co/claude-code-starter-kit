...
STR_PROFILE_FULL="Full - 全機能有効（全フック + Codex プラグイン + Ghostty セットアップ含む）"
STR_PROFILE_FULL_NO_GHOSTTY="Full - 全機能有効（全フック + Codex プラグイン）"
STR_PROFILE_CUSTOM="Custom - 個別にコンポーネントを選択"

# Step 3: Codex Plugin
STR_CODEX_TITLE="OpenAI Codex プラグインを使用しますか？"
STR_CODEX_YES="はい - Codex プラグインをインストールし、委譲ルールを CLAUDE.md に追加"
...
# Step 4: Editor
STR_EDITOR_TITLE="使用しているエディタを選択してください（設定の記録用）："
...
STR_HOOKS_SAFETY_NET="Safety Net - 破壊的な git/ファイルシステムコマンドの偶発的実行を抑止 (cc-safety-net)"
STR_HOOKS_AUTO_UPDATE="自動アップデート - セッション開始時にスターターキットを自動更新"
STR_HOOKS_WEB_CONTENT_UPDATE="Web取得スキル更新 - セッション開始時に web-content-extraction の依存を自動更新（opt-in）"
STR_HOOKS_TMUX="Tmux リマインダー - foreground の dev サーバーに run_in_background を提案（非ブロック）"
STR_HOOKS_DOC_BLOCK="Doc ブロッカー - SUMMARY/REPORT 等のアドホック文書のみ確認（一般文書は素通し）"
...
STR_WCE_NODE_MISSING="Node.js が見つかりません — web-content-extraction の URL/PDF 機能は依存導入まで無効です。"
STR_WCE_NODE_HINT="Node.js 22.19+ を入れて実行: (cd ~/.claude/skills/web-content-extraction && npm ci --omit=dev)"
STR_WCE_NPM_INSTALLING="web-content-extraction スキルの依存をインストール中..."
STR_WCE_NPM_DONE="web-content-extraction の依存をインストールしました"
STR_WCE_NPM_FAILED="web-content-extraction の npm ci に失敗しました（依存導入までスクリプトは動作しません）"
...
STR_CODEX_SETUP_TITLE="Codex プラグインセットアップ"
STR_CODEX_SETUP_NOTE="注意: Codex プラグインは ChatGPT サインインまたは OpenAI API キー認証に対応しています"
...
STR_CODEX_CLI_SUDO_SKIPPED="非対話モードでは sudo を促せません。Codex CLI を手動でインストールしてください。"
STR_CODEX_AUTH_CHECKING="Codex CLI の認証状態を確認中..."
STR_CODEX_AUTH_NOT_LOGGED_IN="Codex CLI は未認証です。認証方法を選択してください"
STR_CODEX_AUTH_NONINTERACTIVE_REQUIRED="Codex CLI の認証には対話モードでのセットアップがまだ必要です。"
...
STR_CODEX_PLUGIN_INSTALLING="Codex プラグインをインストール中..."
STR_CODEX_PLUGIN_INSTALLED="Codex プラグインがインストールされました"
STR_CODEX_PLUGIN_ALREADY="Codex プラグインはインストール済みです"
STR_CODEX_PLUGIN_MARKETPLACE_FAILED="Codex プラグインの marketplace 追加に失敗しました。install は続行します:"
STR_CODEX_PLUGIN_FAILED="Codex プラグインのインストールに失敗しました。手動で実行:"
STR_CODEX_MCP_REMOVE_FAILED="Codex MCP の削除に失敗しました。手動で実行:"
STR_CODEX_MCP_DRIFT_CLEANUP="重複する Codex MCP 登録を解除中..."
STR_CODEX_MCP_DRIFT_DONE="Codex MCP の重複登録を解除しました"
STR_CODEX_MCP_DRIFT_KEEPING="cleanup が完了しなかったため、legacy Codex MCP を維持します。"
STR_CODEX_MCP_KEEP_UNTIL_READY="Codex Plugin のセットアップが完全に整うまで、legacy Codex MCP を維持します。"
STR_CODEX_MIGRATE_PROMPT="Codex MCP が登録されています。プラグインに移行しますか？"
STR_CODEX_MIGRATE_YES="はい、プラグインに移行"
STR_CODEX_MIGRATE_NO="いいえ、MCP を維持"
STR_CODEX_MIGRATE_DONE="Codex MCP → プラグインへの移行が完了しました"
STR_CODEX_MIGRATE_KEEP_MCP="Codex Plugin のセットアップが未完了のため、MCP を維持します。"
STR_CODEX_MIGRATE_DECLINED="Codex MCP を維持します。プラグインはインストールしません。"
STR_CODEX_MIGRATE_SKIP_NONINTERACTIVE="Codex MCP の移行は対話モードで再実行してください"
STR_CODEX_RESTART_HINT="Claude Code が起動中の場合は、新しいターミナルまたは次回セッションで Codex プラグインの反映を確認してください"
STR_CODEX_SETUP_DONE="Codex プラグインのセットアップが完了しました"
STR_CODEX_SETUP_INCOMPLETE="Codex プラグインのセットアップは未完了です。後で残りの手順を実行してください"
STR_CODEX_SETUP_CONFIRM="Codex プラグインのセットアップを開始しますか？"
STR_CODEX_SETUP_CONFIRM_YES="はい、セットアップする"
STR_CODEX_SETUP_CONFIRM_NO="いいえ、スキップする"
STR_CODEX_SETUP_SKIPPED="Codex プラグインのセットアップをスキップしました"
STR_CODEX_PLUGIN_REMOVE_ASK="Codex プラグインを削除しますか？ [y/N] "
STR_CODEX_PLUGIN_REMOVED="Codex プラグインを削除しました"
STR_CODEX_PLUGIN_REMOVE_FAILED="Codex プラグインの削除に失敗しました。手動で実行:"
...
STR_EXISTING_CLAUDE_SIDE_EFFECTS="選択内容によっては Ghostty、フォント、shell RC、Codex プラグインなどの外部副作用も実行される場合があります。"
