...
STR_WCE_NODE_HINT="Node.js 20+ を入れて実行: (cd ~/.claude/skills/web-content-extraction && npm install --omit=dev)"
STR_WCE_NPM_INSTALLING="web-content-extraction スキルの依存をインストール中..."
STR_WCE_NPM_DONE="web-content-extraction の依存をインストールしました"
STR_WCE_NPM_FAILED="web-content-extraction の npm install に失敗しました（依存導入までスクリプトは動作しません）"
...
STR_CODEX_MIGRATE_PLUGIN_FAILED="プラグインのインストールに失敗しました。MCP を維持します。"
