...
STR_CODEX_RESTART_HINT="Claude Code が起動中の場合は、新しいターミナルまたは次回セッションで Codex MCP の反映を確認してください"
STR_CODEX_SETUP_DONE="Codex MCP のセットアップが完了しました"
STR_CODEX_SETUP_INCOMPLETE="Codex MCP のセットアップは未完了です。後で残りの手順を実行してください"
STR_CODEX_SETUP_CONFIRM="Codex MCP のセットアップを開始しますか？（ChatGPT ログインまたは OpenAI API キー認証に対応）"
...
STR_DRYRUN_NO_CHANGES="ファイルは変更されませんでした。これはドライランです。"
STR_DRYRUN_SETTINGS_NEW="新規 settings.json が作成されます:"
STR_DRYRUN_SETTINGS_DIFF="settings.json の差分（現在 → シミュレーション後）:"
STR_DRYRUN_CONFLICT_REMEMBERED="（記憶済み）"
STR_DRYRUN_CONFLICT_WILL_PROMPT="→ 本番では確認プロンプトが出ます"
...
STR_EXISTING_CLAUDE_SIDE_EFFECTS="選択内容によっては Ghostty、フォント、shell RC、Codex MCP などの外部副作用も実行される場合があります。"
