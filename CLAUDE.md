...
5. Update the hardcoded plugin counts and lists in `README.md` (profile table, plugin section heading, plugin table row) and `README.en.md` (feature bullet, profile table), plus the exact profile-count assertions in `tests/unit/test-context-budget.sh`. When the plugin carries operational caveats a user needs *before* running it (execution prerequisites, output residue, cost/consent, coverage limits, trust and credential boundaries), sync those to `README.en.md` too — not just the counts. `.github/instructions/review.instructions.md` treats `README.en.md` prerequisites as a review gate
...
8. Verify adoption for existing installs through `lib/plugin-adoption.sh`: an interactive update offers each newly catalogued profile default individually (default no), while non-interactive updates install nothing and leave a SessionStart notification handled by `/update-kit`. A decline must be remembered, manual `/plugin install <name>@<marketplace>` remains optional, and an accepted plugin requires `/reload-plugins` or a new session
9. Keep adoption state (`KNOWN_PLUGINS`, `DISMISSED_PLUGINS`) in the conf, never the manifest. Keep `KNOWN_PLUGINS` out of `_CONFIG_EMPTY_ALLOWED_KEYS`: its absence is what earns an older install the one-time catch-up
10. MDM is unaffected — managed mode forces `SELECTED_PLUGINS=""` and `mdm/render-expected.py` never reads `config/plugins.json`. Enabling plugins *under* MDM would be a different change: `claude plugin install` writes `enabledPlugins` into the byte-attested `settings.json`
