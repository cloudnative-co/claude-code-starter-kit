...
- **mdm/ bundle（スクリプト配布の実体・MDM の「スクリプト実行」で起動）**:
  ...
  - MDM が単一ファイルしかアップロードできない場合に備え、**launcher が bundle を取得する 2 方式**を提供:
    (a) キット GitHub から `KIT_MDM_GIT_REF` 固定で mdm/ を取得して実行、
    (b) bundle 一式を MDM のスクリプト添付/ペイロードで同時配布（オフライン fleet 向け）
...
U1 キット取得 + ref ピン留め(§5.5) — install.sh は再実行しない。clone と ref 確定を wrapper が管理
...
1. `KIT_MDM_GIT_REF` の形式判定を分離: 40/64桁 hex は SHA として扱う / それ以外は `git check-ref-format` で検証（不正なら exit 50）
...
- 値は型別に検証し、不正値は加工せず exit 50:
  ...
  git ref（§5.5 の判定: SHA or `git check-ref-format`）/ username（OS ユーザー名文字種）/ path（絶対 + 許可プレフィックス配下）
