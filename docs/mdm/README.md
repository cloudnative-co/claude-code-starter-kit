...
**現時点の実装は macOS 13.5 以上のみ**（`mdm/install-mdm.sh` / `mdm/render-expected.py` / `mdm/detect-mdm.sh`）。MDM の Safety Net / Web Content Extraction runtime は公式 Node v24.18.0 を固定し、その upstream binary が要求する最小 OS を下限とする。Windows 版（`install-mdm.ps1` 等）は今後対応予定であり、本ガイドの対象外。

この機能の目的は、MDM が配布した信頼済み bundle と固定 commit SHA から、ユーザー操作なしで Claude Code CLI と starter kit を導入し、指定した profile / language / feature policy へ冪等に収束させ、検知結果から再修復できる状態にすること。MDM 層は本体の `setup.sh` を対象ユーザー権限で再利用し、配備処理を別実装しない。`settings.json` 以外のユーザー所有領域を不用意に消さないこと、非 MDM の通常インストール/update の挙動を変えないことも契約に含む。
...
- renderer は固定 SHA と MDM 設定から、存在すべき managed files（present）の内容・mode、存在してはならない managed files（absent）、必須 runtime components、canonical `policy.json` を静的に生成する。root は配備後の live files / snapshot と runtime components を照合し、`policy.json` の SHA-256 を固定した場合だけ schema v3 レシートを書く。
...
package / policy 作成側では、配布する full SHA を checkout したツリーと、端末へ渡す設定と同じ renderer 引数から policy SHA-256 を作る。`--checkout` / `--output` / `--profile` / `--language` / `--editor` / `--claude-cli-required` / `--logical-home` はすべて必須で、特に editor と Claude CLI 方針を暗黙の既定値に委ねる経路はない。次は installer 既定の `standard` / `en` / `none` / Claude CLI 必須に対応する例。明示する feature 設定は installer の `KEY=VALUE` と renderer の `--override KEY=VALUE` の双方へ同じ値で渡す。
...
`--logical-home` は absolute canonical path が必要だが policy digest には含まれないため、この生成専用 placeholder を使える。生成後は `POLICY_SHA` が小文字64桁であることを検証し、同じ値を remediation の `KIT_MDM_EXPECTED_POLICY_SHA256` と detector の `--expected-policy-sha256` へ配布する。出力ディレクトリは機微情報を含まないが、一時成果物として処分する。
...
| キー | 既定値 | 意味 |
|---|---|---|
| `KIT_MDM_TARGET_USER` | 自動検出（コンソールユーザー） | 明示指定時もローカルアカウント実在・UID>=501・home 所有者一致を検証する。対応する short name は32文字以内の ASCII 英数字・`_`・`-`・dot / `@` 区切り（大文字・数字先頭可、区切りの先頭末尾・連続は不可）。`dscl` home 値自体が canonical physical path でない場合を含め、不一致は `exit 20` |
...
| `KIT_MDM_EXPECTED_POLICY_SHA256` | 未指定 | 小文字64桁 SHA-256。指定時は renderer が現在の profile / language / editor / commit attribution / feature / 必須 component から算出した `policy.json` と一致しなければ、setup 前に `exit 50`。未指定でも算出値は成功レシートへ必ず記録する |
...
- canonical `policy.json` の実 SHA-256 と renderer manifest の `policy_sha256` が一致し、`KIT_MDM_EXPECTED_POLICY_SHA256` 指定時はその値にも一致する
...
MDM 製品側にも全体 watchdog を設定する。通常は30〜60分を目安とし、CLT fallback と runtime build を同じ実行で許可する場合は、上記の複数フェーズが直列になり得るため製品側上限を十分長くする。製品側が先に停止すると launcher 自身の cleanup 完了を待てないため、少なくとも単一の `setup.sh` 上限1200秒より短くしない。
...
対象ユーザー状態への適用は outer transaction とし、既存 `~/.claude`（初回は不在状態）、保持用 checkout、root-owned managed history / component manifest を成功レシート公開まで復元可能に保持する。postcondition / component attestation / receipt 公開の失敗、timeout、HUP / INT / TERM では旧 inode または不在状態と root state を復元し、拒否した候補は `~/.claude.mdm-failed.*` / `~/.claude-starter-kit.mdm-failed.*` に診断用として保持する。rollback 後の失敗レシートは best-effort で発行する。

CLT / Homebrew 導入と hash-versioned root runtime / cache / quarantine はこの rollback 境界外で、検証済みの準備物を単調に保持する。`SIGKILL` と電源断には durable journal がなく trap / rollback を保証しない。発生時は実行停止を確認して candidate / stage / lock を調査し、detector の再評価後に remediation を再実行するか管理者が復旧する。
