...
MDM（Jamf / Intune / Workspace ONE / Ivanti 等）から macOS 13.5 以上の管理端末へ Claude Code CLI とキットをゼロタッチ配布するサイレントインストール機能を追加。Xcode Command Line Tools は MDM baseline として事前配布する構成が既定。Windows 版は今後対応予定。
...
- **macOS account と CLT fallback の互換性を修正**: 大文字・数字先頭・dot / `@` 区切りの対象ユーザー名を installer/config/detector で一貫して受理し、path-like な区切りは拒否する。空白を含む canonical home と `UniqueID` は実 `dscl` 形式から単一値として厳格に解析し、producer の非0終了も拒否する。local / search-policy / home owner の UID を束縛し、失敗した `scutil` の部分出力は採用しない。`softwareupdate -l` の現行 `Label:` 形式と旧形式を正規化して最新 CLT label を渡す
- **対象ユーザーへの降格実行と lifecycle を修正**: local/search の UID / GeneratedUID tuple を束縛し、`sudo -u "#<uid>"`、検証済み home の CWD、実 `UserShell`、clean PATH で setup を実行する。launcher は永続 installer / renderer と一時 snapshot を分離し、HUP / INT / TERM を child process group へ転送して一時物だけを回収し 129 / 130 / 143 で停止する。coordinator が終了しても child の寿命まで remediation lock を保持する
- **MDM 後段失敗の部分適用を修正**: `~/.claude`、保持用 checkout、managed history、component manifest を outer transaction で束ね、postcondition / receipt 書込み失敗、timeout、HUP / INT / TERM では直前の成功状態または初回の不在状態へ復元し、拒否した候補は診断用に保持する。schema / semantics を完全検証した成功 receipt の atomic 公開を commit point とし、公開後の cleanup / backup rotation 失敗で成功状態を巻き戻したり成功終了コードを失ったりしない
- **Intel Mac と Linux CI の互換性を修正**: Intel では存在しない `hw.optional.arm64` OID を空値として扱い、`uname -m` と束縛して native Apple Silicon / Rosetta / Intel を `arm64` / `x64` へ厳格に解決する。Ghostty / fonts の macOS system API テストは非 Darwin CI だけ明示 skip し、Windows の既存テスト経路は維持する
