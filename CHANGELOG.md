...
- **macOS account と CLT fallback の互換性を修正**: 大文字・数字先頭・dot / `@` 区切りの対象ユーザー名を installer/config/detector で一貫して受理し、path-like な区切りは拒否する。`dscl` の canonical home path に含まれる空白を保持し、`softwareupdate -l` の現行 `Label:` 形式と旧形式を正規化して最新 CLT label を渡す
- **MDM 信頼境界テストを実経路へ強化**: production `errexit`、launcher owner/mode/symlink/ACL、config/home owner、production override 隔離、実 `deploy_hook_scripts` と renderer の byte/inventory/mode parity を回帰テスト化
