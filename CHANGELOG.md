...
- **macOS short name と CLT fallback の互換性を修正**: 大文字・数字先頭・dot / `@` 区切りの対象ユーザー名を installer/config/detector で一貫して受理し、path-like な区切りは拒否する。`softwareupdate -l` の現行 `Label:` 形式と旧形式を正規化して最新 CLT label を渡す
