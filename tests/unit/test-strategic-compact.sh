...
XDG_RUNTIME_DIR="$_sc_runtime" COMPACT_THRESHOLD=5 bash "$_sc_script" <<< "$_sc_input_a" >/dev/null 2>/dev/null
XDG_RUNTIME_DIR="$_sc_runtime" COMPACT_THRESHOLD=5 bash "$_sc_script" <<< "$_sc_input_a" >/dev/null 2>/dev/null
XDG_RUNTIME_DIR="$_sc_runtime" COMPACT_THRESHOLD=5 bash "$_sc_script" <<< "$_sc_input_b" >/dev/null 2>/dev/null

if assert_equals "2" "$(cat "$_sc_runtime/tool-count-session-a")" \
...
XDG_RUNTIME_DIR="$_sc_runtime" COMPACT_THRESHOLD=5 bash "$_sc_script" <<< "$_sc_input_a" >"$_sc_out" 2>"$_sc_err"

if assert_equals "$_sc_input_a" "$(cat "$_sc_out")" \
