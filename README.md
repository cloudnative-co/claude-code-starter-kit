...
> Codex Plugin（OpenAI 連携）を使う場合は、別途 ChatGPT の対象プラン契約、または OpenAI API キー認証と OpenAI API の利用料金が必要です。
...
- **おすすめ**: ほとんどの人に最適
- **含まれるもの**: エージェント、ルール、コマンド、スキル、主要なフック
- **特徴**: バランスの良い定番セット
...
- **Node.js 22.19+ が必要**（22/24 でテスト）。セットアップは不足時に自動導入・更新を試み、最低版を満たせなければ配備前に失敗します。導入時に `npm ci --omit=dev` が自動実行されます。
...
| フック | 何をしてくれる？ |
|---|---|
| **Safety Net** | **破壊的な git/ファイルシステムコマンド（`git reset --hard`、`rm -rf` 等）の偶発的な実行を抑止** |
...
[cc-safety-net](https://github.com/kenryu42/cc-safety-net) は、Claude Code が実行しようとする Bash コマンドを **事前にチェックして、偶発的な破壊的操作を抑止する** 安全装置（footgun 防止）です。コマンド文字列のパターンマッチングに基づくため、意図的な回避やプロンプトインジェクション対策としては設計されていません。
...
STRICT モード（`CC_SAFETY_NET_STRICT=1`）が有効で、パース不能なコマンドも fail-closed（ブロック）になります。

> **Standard / Full プロファイルでデフォルト有効です。** 未導入の場合はセットアップが `cc-safety-net` を npm で自動インストールします（lifecycle script を実行しない `--ignore-scripts` 付き）。自動インストールに失敗した場合は `npm install -g --ignore-scripts cc-safety-net` を手動で実行してください。
...
| **pr-review-toolkit** | 専門エージェントによる包括的な PR レビュー | Full |
| **feature-dev** | アーキテクチャ重視の機能開発ガイド | Standard / Full |
| **code-review** | プルリクエストのコードレビュー | Standard / Full |
| **claude-md-management** | CLAUDE.md の監査・改善 | Full |
| **superpowers** | ブレスト、TDD、デバッグ等のスキルシステム | Full |
| **code-simplifier** | コード簡素化・リファクタリング支援 | Standard / Full |
| **document-skills** 🆕 | ドキュメント作成・編集スキル（DOCX, PDF, PPTX, XLSX） | Full |
| **example-skills** 🆕 | Anthropic 公式のクリエイティブ・技術・エンタープライズスキル集 | Full |
...
> - **`python3` 3.9 以上**が PATH 上に必要です。キットの前提ツール確認は `python3` を対象にしていないため、不足していても導入時には検出されません（macOS 標準の `/usr/bin/python3` は要件を満たします）。
...
> - **非対話セッションでは動きません。** 実行前にトークン消費の確認プロンプトが入り、答えられない場合は何も生成せず停止します。CI 用途には使えません。
...
> - キットの deny ルールの都合で、**ファイル名が `.env` / `secrets/` / `*secret*` / `*credential*` に一致するファイル**は Read と、そのパスを参照する Bash コマンドが拒否されます。ファイル名ベースの制限なので内容ベースの保護ではありませんが、この範囲についてはスキャンの取りこぼしが起こりえます。
>
> **既存の環境には自動で追加されません。** アップデート（`/update-kit` や `setup.sh --update`、ワンライナーの再実行）では、導入済みプラグインの一覧がマニフェストから復元されるため、新しく追加されたプラグインは取り込まれません。すでに Full でインストール済みの方は、Claude Code 上で `/plugin install claude-security@claude-plugins-official` を実行してください。
...
> 非対話モードでは `name@marketplace` 形式（例: `document-skills@anthropic-agent-skills`）で指定できます。
> 現在の既定プラグインに名前の衝突はありません。
...
> 衝突しないプラグインは従来通り名前だけで指定できます（例: `security-guidance`）。`name@marketplace` 形式は衝突がなくても使えます。
>
> **補足**:
...
> - ドライラン中は `Preview Mode` として表示されるため、本番 update と見分けやすくなっています。
...
Jamf / Intune / Workspace ONE / Ivanti などの MDM 経由で、macOS 13.5 以上の社内端末へ Claude Code CLI とこのキットを利用者への質問なしで配布できます。Xcode Command Line Tools は MDM の pkg 機能で先に配布する baseline が既定です。production remediation は root、`KIT_MDM_GIT_REF` の小文字40桁 commit SHA、`KIT_MDM_EXPECTED_POLICY_SHA256` の小文字64桁 policy SHA-256 が必須で、同じリリースの `mdm/install-mdm.sh` + `mdm/render-expected.py` を trust bundle として配布します。MDM 層は本体の `setup.sh` を再利用し、同じ SHA の冪等な再実行でも改変・欠落を期待状態へ修復します。trusted static renderer が present / absent、必須 runtime components、desired policy を生成し、root が postcondition を確認した場合だけ policy SHA-256、UID / GeneratedUID、runtime component manifest の path/hash を含む schema v3 receipt を発行します。

`settings.json` は全体を MDM 管理し、`CLAUDE.md` の user section と別のユーザーファイルは保持します。MDM では auto-update、web updater、通常 plugin、Codex Plugin を常に無効化し、Ghostty / fonts だけを明示 opt-in にできます。global remediation lock、UID / GeneratedUID に束縛した root history、最新1世代の専用バックアップで再実行を保護します。Apple 署名済み CLT Python は、root 実行経路では全期間 root-owned mode `0700`、明示的な non-root dry-run だけは非権威・receipt 不変の current-user-owned mode `0700` ephemeral workspace へコピーし、署名・metadata・self-test・full seal を照合して使います。対象ユーザーへ降格する3つの user-owned 操作は private copy を実行せず、検証済み fixed source を使い、production では後段の root-private postcondition で再検証します。こちらでは署名せず Developer ID 証明書・秘密鍵も使いません。production の検知は root-only で、expected commit と expected policy SHA-256 の両方を要求します。製品側 watchdog は30〜60分が初期目安ですが完了保証ではありません。詳細と実 MDM 製品からの root end-to-end 未検証事項は [`docs/mdm/README.md`](docs/mdm/README.md) を参照してください。**MDM 実装は macOS 13.5 以上のみで、Windows MDM は対象外**です。
...
### Q. Codex Plugin って何ですか？必要ですか？

Codex Plugin は **OpenAI の Codex（AI コーディングツール）を Claude Code のサブエージェントとして連携する仕組み** です。
...
3. OpenAI API キーで使う場合のみ、API キーを追加してログイン：
   ...
   ```
4. Claude Code に plugin を追加：
   ```bash
   claude plugin marketplace add openai/codex-plugin-cc
   claude plugin install codex --scope user
   ```
